Shader "Aulas/alphHologramFullShaderRotation"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        pass{// pass do render
            ZWrite On ZTest Less ColorMask B
        ...
        float3 RoateY(float3 pos , half angle)
        {
            float c = cos(angle);
            float s = sin(angle);

            return mul(float3x3(c,0,s,0,1,0,-s,0,c), pos);
        }


        void vert(inout appdata_full v)
        {
            v.vertex.xyz = RoateY(v.vertex.xyz, _Time.z);
        }
